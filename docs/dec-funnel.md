...
**R0 — Every ratio ships with its convention named.** R1–R14 are all special cases. A measured number carries a hidden condition — the corpus it was scored on, the activation fraction it was measured at, the component it was divided by, the bytes it assumed present, the units it was expressed in, the execution it presumed. State the condition *with* the number, every time, or the next person to reuse it (including you, next week) reuses it somewhere the condition doesn't hold.
...
**R12 — Name the metric's SPACE and the search's GUARANTEE. Both, every time, on every programme.** This one is not DEC-local: it binds RSL, DEC, VINDEX and K3 alike, because it governs the *naming* of results rather than any particular measurement. Skipping either half promotes a measurement to the strongest nearby noun.

*Space.* State which space the metric lives in, and — for anything precision-sensitive — which **side** of the comparison each precision applies to. Two measurements, same day, same model (Gemma-3-4B, GLA-4 audit 2026-08-03/04). First, alignment of two error sources: raw logit L2 says `cos = +0.0475` (near-orthogonal), the Fisher metric `G(p) = diag(p) − p pᵀ` says `cos = −0.9884` (almost exactly anti-aligned) — *same two vectors*, and centring alone does not reveal it, because softmax is invariant to common-mode shifts. Second, the cap_france 9-cell route re-scored across the reference × candidate precision matrix — **one fixed cell set, four numbers**:

| | candidate bf16 | candidate fp32 |
|---|---:|---:|
| **reference bf16** | 0.0113 b ✓ | 0.0646 b ✗ |
| **reference fp32** | **0.0194 b ✓** | 0.0823 b ✗ |

A **7.3× spread** and a flipped verdict from nothing but which side ran in which dtype, against a dense-path precision floor of **0.0011 b** — sparsification amplifies the bf16/fp32 gap ~75× relative to the dense path, so "fp32 scoring" is ambiguous exactly where it matters. **Corollary (deployment-path qualification):** execute the candidate on the path that will ship, score it against a canonical fp32 reference — the bolded cell. An audit that moves *both* sides to fp32 is measuring a counterfactual deployment, and one that moves neither is measuring shared rounding.

**"fp32 scoring" is not a method description — record an execution fingerprint.** The same audit found *four* separately controllable operations, not two, and "head precision" is itself an umbrella term: promoting `final_norm` alone moved the numbers **4.6–11%** (canonical 0.031719 → 0.033186; deployment-relative 0.031818 → 0.035351) and accounted for the entire discrepancy between two sessions' figures for the identical route. Every thresholded experiment records:

    reference:   body=fp32  final_norm=fp32  unembedding=fp32
    candidate:   body=bf16  final_norm=bf16  unembedding=fp32
    metric:      KL(reference || candidate), bits, full-vocabulary softmax
    trajectory:  teacher-forced
    evaluation:  unbatched

The last line is not boilerplate. Batched evaluation on Gemma-3-4B/MLX differs from unbatched by **max |Δlogit| 0.125** at B=8 — enormous next to decisions sitting 0.001–0.008 b from their boundary — so a 3.32× speedup would inject a perturbation in the same currency as the effect under study. Batching is legitimate as a *screen*, but anything near threshold needs unbatched qualification, and using unbatched throughout keeps the proof clean.

*Guarantee.* Keep four levels apart and never promote between them: **feasible** (metric inside the declared contract) → **greedy-selected** (produced by the specified search order) → **inclusion-minimal** (no single selected member removable) → **minimum-cardinality** (no smaller feasible solution exists anywhere). Only exhaustive enumeration or valid branch-and-bound earns the last — and **exhaustive *within a coarse bucket* earns nothing globally unless the bucket is proven to bracket the answer.** Measured the same day: a global C(58,1..3) sweep found minimum-cardinality **3** with nine routes, of which **seven sit at position sets that were not qualifying coarse addresses at all**, while the within-address answer (4) came from taking `addresses[0]` out of **ten** equally-valid tie-breaks. Worse, four of the nine live entirely at positions {17,18} yet restoring *all twelve* cells at those two positions **fails** — so the coarse statistic ("restore the whole bucket") is non-monotone in the fine one and cannot bracket it. Report which you did: proved monotonicity, searched globally to a stated bound, or carried **all** coarse winners forward. If ties exist at the coarse stage, the tie count is part of the result. Measured cost of skipping this: birth_einstein's published `VERDICT: MANDATORY MINIMAL SET (unique): [1,7,13,15]` came from a search that **stopped at the first rescuing cardinality**, where the lone rescuing set cleared its threshold by **0.0014 b** (0.0486 against 0.05). Re-run exhaustively over all 64 position-9 subsets under deployment-path scoring: **zero** rescuing subsets — the best achievable is 0.0724 b, 1.45× the threshold. Uniqueness, cardinality, and *existence* failed together, and a stopping rule cannot detect that. Related: a low-margin deterministic decision is a **low-margin, quantisation-sensitive decision boundary**, never a "coin flip". Instruments: `chris-experiments/rsl/exp14b_precision_matrix_capfrance.py`, `exp15_gla4_corrected_route_capfrance.py`, `exp16_gla4_teacher_forced_birtheinstein.py`.

**R13 — Route membership is selected and validated JOINTLY. Isolated marginal importance is not a safe pruning criterion.** A member with zero or negative isolated benefit can be *necessary* within a qualifying execution route, so an independent per-layer/per-expert importance score cannot identify safe drop candidates. Formally, for a member `i` and a route `R`, `F({i}) − F(∅) < 0` does **not** imply `F(R∖{i}) − F(R) ≤ 0`. Measured on Gemma-3-4B (GLA-4 audit, 2026-08-04, cap_france corrected mask): cell `(3,7)` has the **worst isolated effect available, −0.732 b** — restoring it alone is the single most damaging move — yet it is necessary in **all four tested inclusion-minimal routes**, each of which fails without it (0.124 / 0.156 / 0.215 b). Same shape at `(2,7)` (−0.170 b) and `(7,5)` (−0.080 b): **11 of 34 membership tests pair a negative isolated effect with conditional necessity**, while cells with a healthy +0.179 b isolated effect proved droppable. So marginal ranking here is not merely weak but **anti-predictive** — it sorts the mandatory members to the top of the drop list.

**Corollary A — RESIDENT set and EXECUTION set are different objects, and this rule constrains only the second.** The non-monotonicity above (restoring all twelve cells at positions {17,18} fails where three of them qualify) is a statement about *semantic materialisation*, not about *what may sit in cache*. Fetching all twelve is **behaviourally** harmless provided the runtime activates only the qualified three. So: **physical fetch granularity may stay coarse; semantic activation granularity must be fine.** Concretely for K3 — page in a slab containing several experts or blocks, execute only the qualified route, let residency be a strict superset of the execution set, and attach route qualification to the **activation mask**, never to cache contents. Reading the non-monotonicity as "caching extra state is dangerous" would be a serious and expensive misdiagnosis: it would forbid exactly the coarse page I/O that makes the read side affordable, while leaving the actual defect — coarse *activation* — untouched. **"Harmless" here means behaviourally harmless, never cost-free** (R3): unused fetched bytes, decompression/transcode work, cache pollution and the eviction of more valuable slabs are all still on the bill, and the planner prices them.

**Corollary B — a route family's core is relative to its objective, so do not plan a cache around minimum-cardinality routes alone.** Measured on the same mask: `(12,17)` is in **9/9** minimum-cardinality (3-cell) routes, an exact singleton intersection — yet **all four** inclusion-minimal 8–9-cell routes qualify while omitting it entirely. Different objective, different core. And fewest *activations* is not fewest *bytes*: a 3-cell route whose core is cold can cost three page reads where an 8-cell route that is already wholly resident costs zero, so the larger route wins outright. The runtime objective is therefore `argmin_R C_physical(R | resident state)` over the qualified set subject to the fidelity contract — **not** `argmin |R|`. Minimum-cardinality families remain valuable for exposing strong structural alternatives; they are not the thing to prefetch against.

This is a rule about **policy correctness, not a statistical universality claim**, and it binds DEC, VINDEX and K3 wherever a pruning, retirement, or execution-set decision is made: qualify each candidate drop *conditionally, against the route it will actually execute in*, never against a solo score. Scope of the evidence, stated exactly: one item, four tested routes; `(3,7)` is not thereby proven globally mandatory across every feasible route. The claim previously rested on two items — birth_einstein's L1/L15 no longer supports it, because under corrected scoring that item has **no qualifying route at all**, which makes its necessity test vacuous rather than confirmatory. One sharper instance replaces two weaker ones. See `feedback_marginal_importance_not_necessity` and RSL-3, which this re-scopes from "test whether" to "characterise when it inverts".

**R14 — Gate–claim congruence. A validity gate licenses only claims over the same object, trajectory and counterfactual relation that the gate itself tests.** A gate covering a strict subset of the claimed object is a *local sanity check*, not a certificate. Both gate failures in the EXP-21/22 sequence were congruence failures — the gate did not malfunction, it was asked a narrower question than the conclusion drew on:

| claim quantifies over | gate must cover |
|---|---|
| a single state | exact state parity |
| a multi-step trajectory | **every step in that trajectory** |
| a growing/mutating cache | invariants re-checked after **every** mutation |
| a matched counterfactual | exact equality **outside** the declared intervention |
| semantic equivalence | the declared output/trajectory metric, not a proxy |
| physical equivalence | identical execution fingerprint (R12) |
| all target classes | **every admitted class**, not the easy ones |

**Temporal mismatch, measured (2026-08-04):** a 4-token greedy comparison passed while the gather path was silently dropping every newly appended answer token; teacher-forced scoring over the full answer span showed **max KL 18.4 bits** and a garbage continuation. The first decode step agreed in every arm — the divergence began at step 2, and the 29 sliding layers masked it in greedy sampling. **Construction mismatch, measured the same day:** "a counterfactual was built" was used to license "the counterfactual differs only in the declared intervention"; in fact an advanced RNG gave the two contexts different filler across all 65K tokens, making a 1.006-bit KL uninterpretable.

**Corollary — snapshot coordinates must not be reused against a mutating state space.** The concrete defect: a keep-index set derived at a pre-answer snapshot, applied unchanged as the KV cache grew, so every new token fell outside the selection. This class recurs wherever indices outlive the thing they index — KV growth during decode, rotating/compacting caches, VINDEX page remapping, expert residency changing between tokens, batch members retiring at different times. **Selectors should address stable logical identities** (logical position, layer, page/record identity, generation epoch) and resolve to physical indices at execution time; recomputing from the live extent, as here, is the append-only special case.

**Two gate families, and a mature harness needs both.** *Intervention gates* verify the manipulation does what it claims — these caught the silent no-op patch, the cache-offset mismatch, non-causal prefill, the scoring-position artifact and a first-token accidental pass. *Construction invariants* verify the objects compared are the objects intended — these are what the RNG confound and the snapshot-index reuse needed, and neither existed. Run `intervention validity + construction identity + trajectory congruence`, not "controls".
