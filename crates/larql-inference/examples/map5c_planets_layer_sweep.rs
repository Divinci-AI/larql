...
fn print_scores(label: &str, scores: &[StepScore], discriminating: &[bool], tokenizer: &tokenizers::Tokenizer) {
    print!("    {label:>20}: ");
    ...
    for (s, &disc) in scores.iter().zip(discriminating) {
        ...
        let marker = if disc { if won { "OK" } else { "MISS" } } else { "  " };
        print!("[{text:?} kl={:.2} rank_d={:>3} {marker:>4}] ", s.kl_vs_donor, s.rank_of_donor_token);
    }
    let any_disc = discriminating.iter().any(|&d| d);
    println!(" -> {}", if !any_disc { "no discriminating steps".to_string() } else if all_discriminating_won { "WINS every discriminating step".to_string() } else { "misses at least one discriminating step".to_string() });
}
...
fn main() {
    ...
    let (donor_seq, _donor_rows_unused, donor_dists) = run_reference(&weights, &ffn, &donor_tokens, last_layer, last_layer, N_STEPS);
    ...
    let (_, _, recipient_dists_baseline) = run_reference(&weights, &ffn, &recipient_tokens, last_layer, last_layer, N_STEPS);
    let discriminating: Vec<bool> = recipient_dists_baseline
        .iter()
        .enumerate()
        .map(|(step, dist)| kl_divergence(&dense_from_sorted(&donor_dists[step], weights.vocab_size), &dense_from_sorted(dist, weights.vocab_size)) > DISCRIMINATING_KL_THRESHOLD)
        .collect();
    ...
    for &layer in &LAYER_SWEEP {
        ...
        let (_, donor_rows, _) = run_reference(&weights, &ffn, &donor_tokens, layer, last_layer, N_STEPS);
        let (_, _recipient_rows, _) = run_reference(&weights, &ffn, &recipient_tokens, layer, last_layer, N_STEPS);

        let one_shot = run_condition(&weights, &ffn, &recipient_tokens, layer, last_layer, N_STEPS, &donor_dists, &donor_generated_tokens, None, Some(&donor_rows[0]));
        print_scores("one-shot", &one_shot, &discriminating, &tokenizer);

        let repeated = run_condition(&weights, &ffn, &recipient_tokens, layer, last_layer, N_STEPS, &donor_dists, &donor_generated_tokens, Some(&donor_rows), None);
