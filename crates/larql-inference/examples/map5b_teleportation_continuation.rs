...
const PAIRS: [(&str, &str, usize); 5] = [
    ...
    ("The largest planet in the solar system is", "The smallest planet in the solar system is", 16),
    ("The opposite of hot is", "The opposite of up is", 24),
    ("The chemical symbol for water is", "The chemical symbol for gold is", 24),
...
fn plain_step(weights: &ModelWeights, ffn: &dyn FfnBackend, tokens: &[u32], capture_layer: usize, last_layer: usize) -> (u32, Vec<(u32, f32)>, Vec<f32>) {
    let layers = if capture_layer == last_layer { vec![last_layer] } else { vec![capture_layer, last_layer] };
    let trace = trace_forward_full(weights, tokens, &layers, false, 0, false, ffn);
    let capture_row = trace.residuals.iter().find(|(l, _)| *l == capture_layer).unwrap().1.clone();
    let final_row = trace.residuals.iter().find(|(l, _)| *l == last_layer).unwrap().1.clone();
...
fn patched_step(weights: &ModelWeights, ffn: &dyn FfnBackend, tokens: &[u32], patch_layer: usize, patch_row: &[f32], last_layer: usize) -> (u32, Vec<(u32, f32)>) {
    ...
    let final_row = &trace.residuals.iter().find(|(l, _)| *l == last_layer).unwrap().1;
    let dist = logit_lens_topk(weights, final_row, weights.vocab_size);
    (dist[0].0, dist)
}
...
fn run_reference(weights: &ModelWeights, ffn: &dyn FfnBackend, prompt_tokens: &[u32], capture_layer: usize, last_layer: usize, n_steps: usize) -> (Vec<u32>, Vec<Vec<f32>>, Vec<Vec<(u32, f32)>>) {
...
fn score_step(dist: &[(u32, f32)], donor_dist: &[(u32, f32)], donor_token: u32, vocab: usize) -> StepScore {
    StepScore {
        token: dist[0].0,
        kl_vs_donor: kl_divergence(&dense_from_sorted(donor_dist, vocab), &dense_from_sorted(dist, vocab)),
...
fn print_scores(label: &str, scores: &[StepScore], donor_tokens: &[u32], tokenizer: &tokenizers::Tokenizer) {
    let split_at = scores.iter().zip(donor_tokens).position(|(s, &d)| s.token != d);
    print!("  {label:>28}: ");
    for s in scores {
        let text = tokenizer.decode(&[s.token], false).unwrap_or_default();
        print!("[{text:?} kl={:.3} rank_d={:>4}] ", s.kl_vs_donor, s.rank_of_donor_token);
    }
    println!(" split_at_step={}", split_at.map(|s| s.to_string()).unwrap_or_else(|| "never (matches donor throughout)".into()));
}
...
fn main() {
    ...
    for &(donor_text, _, patch_layer) in &PAIRS {
        ...
        let (seq, rows, dists) = run_reference(&weights, &ffn, &donor_tokens, patch_layer, last_layer, N_STEPS);
        donor_seqs.push(seq);
    ...
    for (i, &(donor_text, recipient_text, patch_layer)) in PAIRS.iter().enumerate() {
        ...
        println!("  donor reference continuation: {:?}", donor_generated_tokens.iter().map(|&t| tokenizer.decode(&[t], false).unwrap_or_default()).collect::<Vec<_>>());

        // Recipient baseline (unpatched) — also gives us the natural row per
        // step, needed to size the magnitude-matched random control.
        let (_, recipient_rows, recipient_dists) = run_reference(&weights, &ffn, &recipient_tokens, patch_layer, last_layer, N_STEPS);
        let baseline_scores: Vec<StepScore> = recipient_dists.iter().enumerate().map(|(step, dist)| score_step(dist, &donor_dists_all[i][step], donor_generated_tokens[step], weights.vocab_size)).collect();
        print_scores("recipient baseline", &baseline_scores, &donor_generated_tokens, &tokenizer);

        // One-shot: patch step 0 with the donor's REAL step-0 row, free-run after.
        let one_shot = run_condition_generation(&weights, &ffn, &recipient_tokens, patch_layer, last_layer, N_STEPS, &donor_dists_all[i], &donor_generated_tokens, None, Some(&donor_rows_all[i][0]));
        print_scores("one-shot teleport", &one_shot, &donor_generated_tokens, &tokenizer);

        // Repeated: patch every step with the donor's own reference row for that step.
        let repeated = run_condition_generation(&weights, &ffn, &recipient_tokens, patch_layer, last_layer, N_STEPS, &donor_dists_all[i], &donor_generated_tokens, Some(&donor_rows_all[i]), None);
        print_scores("repeated teleport", &repeated, &donor_generated_tokens, &tokenizer);
        ...
        let delta_mag = norm(&donor_rows_all[i][0].iter().zip(recipient_rows[0].iter()).map(|(d, r)| d - r).collect::<Vec<f32>>());
        let rand_dir = random_unit_vector(recipient_rows[0].len(), 30_000 + i as u64);
        let control_row: Vec<f32> = recipient_rows[0].iter().zip(rand_dir.iter()).map(|(r, d)| r + d * delta_mag).collect();
        let control = run_condition_generation(&weights, &ffn, &recipient_tokens, patch_layer, last_layer, N_STEPS, &donor_dists_all[i], &donor_generated_tokens, None, Some(&control_row));
        print_scores("magnitude-matched random", &control, &donor_generated_tokens, &tokenizer);
        ...
        let wrong = run_condition_generation(&weights, &ffn, &recipient_tokens, patch_layer, last_layer, N_STEPS, &donor_dists_all[i], &donor_generated_tokens, None, Some(wrong_row));
        print_scores(&format!("wrong-donor (pair {wrong_i})"), &wrong, &donor_generated_tokens, &tokenizer);
    }
