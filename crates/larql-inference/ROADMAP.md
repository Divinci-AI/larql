...
**Tests.** 1423 lib tests passing (`cargo test -p larql-inference --lib`);
192 source files.

**KV engines — 9, not 4.** They live in `larql-kv`, not here: `standard`,
`no_cache`, `apollo`, `boundary_kv`, `boundary_per_layer`, `markov_residual`,
`markov_residual_codec`, `turbo_quant`, `unlimited_context`. This file long
recorded "4 KV engines"; that count dates from the original trait landing.
Engine-level detail, including which engines support remote MoE and which
dispatch path a given run takes, is owned by
[`larql-kv/ROADMAP.md`](../larql-kv/ROADMAP.md) — **read that before comparing
engine numbers**, because on the coarse dispatch path several engines execute
the same kernels and ranking them against each other there measures nothing.

**Coverage.** The policy here is deliberately narrow: a 90% per-file gate over
the dispatch trait families only, with no crate-wide total gate (`total` is 0).
Every file in scope clears the default, and there are no debt baselines. The
crate's overall line coverage (~70% at the time the policy was written)
reflects an older surface tracked separately — do not read "policy passes" as
"the crate is 90% covered".

**Performance.** The headline numbers this file used to carry — 83.2 tok/s
Metal Q4K on Gemma 3 4B, 18.9 tok/s on Gemma 4 26B-A4B MoE, 6.5 tok/s on the
...
- **P0 — `FfnBackend::forward` is infallible.** Raised 2026-05-28, **confirmed
  still open 2026-08-04**: the trait method at `larql-compute/src/ffn.rs:39`
  returns a bare `Array2<f32>`, so a served path has no way to propagate a
  failure and must abort the process instead. `ffn/remote/http.rs:570` still
  does exactly that — `.unwrap_or_else(|e| panic!(…))` on any remote-shard
  network blip mid-generation. The Q4K CPU dequant sites in
  `vindex/kquant_forward/` are the same class.

  This is the highest-leverage fix in the workspace: the signature change also
  unblocks the `larql-compute` MoE panic sites. The sibling
  `interventions.rs` already propagates into `GenerateError`, so the target
  shape is established. Full finding in [`CHANGELOG.md`](CHANGELOG.md)
  2026-05-28.
...
> Ordering below is from 2026-05-10 and has not been re-prioritised since.
> Items 1 and 3 are marked shipped in place; the rest are unverified against
> the current tree.
...
> **Stale as written; corrected 2026-08-04.** This said "the four KV-cache
> engines shipped in `engines/kv_engines/` all reach ~93-95 tok/s on Gemma 3 4B
> using the Metal Q4K path (matching Ollama within 6%)". Two things changed:
> there are now **nine** engines, and they live in the `larql-kv` crate, not
> `engines/kv_engines/` here. The tok/s figure is a 2026-05 measurement.
>
> More importantly, a same-machine ranking across engines is only meaningful
> once you know which dispatch path the run took — on the coarse path several
> engines execute identical kernels. See
> [`larql-kv/ROADMAP.md`](../larql-kv/ROADMAP.md) before reading any
