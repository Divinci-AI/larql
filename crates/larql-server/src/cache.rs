...
impl DescribeCache {
    ...
    pub fn key(model_id: &str, entity: &str, band: &str, limit: usize, min_score: f32) -> String {
        Self::key_scoped(None, model_id, entity, band, limit, min_score, false, 0.0, false, "embedding")
    }
...
mod tests {
    ...
    fn key_format() {
        ...
        assert_eq!(key, "-:gemma-3-4b-it:France:knowledge:20:5:0:0:0:embedding");
    }
    ...
    fn coherence_is_part_of_the_key() {
        ...
        let raw = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 0.0, false, 0.0, false, "embedding");
        let coh = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 0.0, true, 0.0, false, "embedding");
        let filt = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 0.0, true, 0.3, false, "embedding");
        ...
        let rel = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 0.0, true, 0.0, true, "embedding");
        ...
        let res = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 0.0, false, 0.0, false, "residual");
        let emb = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 0.0, false, 0.0, false, "embedding");
        assert_ne!(res, emb, "query modes must not share an entry");
    }

    #[test]
    fn key_scoped_separates_sessions_and_global() {
        let global = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 0.0, false, 0.0, false, "embedding");
        let a = DescribeCache::key_scoped(Some("tenant-a"), "m", "Paris", "all", 20, 0.0, false, 0.0, false, "embedding");
        let b = DescribeCache::key_scoped(Some("tenant-b"), "m", "Paris", "all", 20, 0.0, false, 0.0, false, "embedding");
        ...
        assert_eq!(
            a,
            DescribeCache::key_scoped(Some("tenant-a"), "m", "Paris", "all", 20, 0.0, false, 0.0, false, "embedding")
        );
    }
    ...
    fn key_matches_key_scoped_with_no_session() {
        assert_eq!(
            ...
            DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 0.0, false, 0.0, false, "embedding")
        );
    }
}
