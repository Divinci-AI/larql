...
impl DescribeCache {
    ...
    pub fn key(model_id: &str, entity: &str, band: &str, limit: usize, min_score: f32) -> String {
        Self::key_scoped(None, model_id, entity, band, limit, limit, min_score, false, 0.0, false, true, "embedding")
    }
...
mod tests {
    ...
    fn key_format() {
        ...
        assert_eq!(key, "-:gemma-3-4b-it:France:knowledge:20:20:5:0:0:0:1:embedding");
    }
    ...
    fn coherence_is_part_of_the_key() {
        ...
        let raw = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 20, 0.0, false, 0.0, false, true, "embedding");
        let coh = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 20, 0.0, true, 0.0, false, true, "embedding");
        let filt = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 20, 0.0, true, 0.3, false, true, "embedding");
        ...
        let rel = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 20, 0.0, true, 0.0, true, true, "embedding");
        assert_ne!(coh, rel, "relabel on/off must not share an entry");
        // The two query modes answer different questions about the same
        // entity; sharing a slot would hand a lexical answer to a caller who
        // asked what the model actually computes.
        let res = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 20, 0.0, false, 0.0, false, true, "residual");
        let emb = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 20, 0.0, false, 0.0, false, true, "embedding");
        assert_ne!(res, emb, "query modes must not share an entry");
    }

    #[test]
    fn key_scoped_separates_sessions_and_global() {
        let global = DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 20, 0.0, false, 0.0, false, true, "embedding");
        let a = DescribeCache::key_scoped(Some("tenant-a"), "m", "Paris", "all", 20, 20, 0.0, false, 0.0, false, true, "embedding");
        let b = DescribeCache::key_scoped(Some("tenant-b"), "m", "Paris", "all", 20, 20, 0.0, false, 0.0, false, true, "embedding");
        ...
        assert_eq!(
            a,
            DescribeCache::key_scoped(Some("tenant-a"), "m", "Paris", "all", 20, 20, 0.0, false, 0.0, false, true, "embedding")
        );
    }
    ...
    fn key_matches_key_scoped_with_no_session() {
        assert_eq!(
            ...
            DescribeCache::key_scoped(None, "m", "Paris", "all", 20, 20, 0.0, false, 0.0, false, true, "embedding")
        );
    }
}
