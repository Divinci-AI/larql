...
Dated record of what changed and why. The reasoning is kept in full
because it is the part that is reusable; several entries below are the
only place a diagnosis (and its wrong turns) is written down.

For what is true *now*, and what is planned, see [ROADMAP.md](ROADMAP.md).
