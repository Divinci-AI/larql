...
fn main() -> Result<(), String> {
    ...
    println!(
        "[rung 1] full (128 experts) vs minimal ({} routed experts) — must be bit-identical",
        incumbent_ids.len()
    );
    ...
    let minimal_out = execute(&minimal_op, inputs())
        .map_err(|e| format!("rung 1 minimal population should not error: {e}"))?;
    let rung1_pass = full_out == minimal_out;
    println!(
        "  {}",
        if rung1_pass {
            "PASS — extra unselected experts in the bank change nothing"
        } else {
            "FAIL — full and minimal populations disagree"
        }
    );
    all_pass &= rung1_pass;
    ...
    let without_one: Vec<usize> = incumbent_ids
        .iter()
        .copied()
        .filter(|&e| e != removed)
        .collect();
    println!(
        "\n[rung 2] remove routed expert {removed} (bank: {} of {} routed experts) — must refuse",
        without_one.len(),
        incumbent_ids.len()
    );
    ...
    println!(
        "  {}",
        if rung2_pass {
            "PASS — refused with the exact expert/resident/population it should"
        } else {
            "FAIL — wrong error, wrong metadata, or (worst case) a silent numeric result"
        }
    );
    all_pass &= rung2_pass;

    // ── Rung 3: pad back to full byte capacity with an UNSELECTED expert ────
    let substitute = (0..moe.num_experts)
        .find(|e| !incumbent_ids.contains(e))
        .ok_or("no unselected expert exists to substitute")?;
    ...
    println!(
        "  {}",
        if rung3_pass {
            "PASS — capacity is not residency: still refused for the missing operand"
        } else {
            "FAIL — a byte-capacity-matched wrong substitute satisfied residency"
        }
    );
    all_pass &= rung3_pass;
    ...
    println!(
        "  {}",
        if rung4_pass {
            "PASS — exact parity restored"
        } else {
            "FAIL — restoring the exact operand did not restore the exact output"
        }
    );
    all_pass &= rung4_pass;
