...
use crate::format::vindex3::opplan::exec::gated_delta::{
    recurrence_step, recurrence_step_mutated, GatedDeltaState, Mutation, RecurrenceStep,
};
...
/// What the bf16 L2-norm path costs, for the record. The same
/// implementation is checked against both references so a future change
/// that quietly moved this would show up.
const BF16_PATH_REL_RMS: f32 = 6e-3;
...
fn the_recurrence_and_its_state_match_hf_on_real_weights() {
    ...
    for layer in LAYERS {
        println!("layer {layer}:");
        let q = read_f32(&dir, &format!("L{layer}_q.f32"));
        let k = read_f32(&dir, &format!("L{layer}_k.f32"));
        let v = read_f32(&dir, &format!("L{layer}_v.f32"));
        let g = read_f32(&dir, &format!("L{layer}_g.f32"));
        let beta = read_f32(&dir, &format!("L{layer}_beta.f32"));
        ...
        for t in 0..tokens {
            ...
            println!(
                "  {:34} rel_rms {:.3e}  (bf16 L2-norm path)",
