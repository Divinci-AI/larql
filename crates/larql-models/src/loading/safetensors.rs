    ...
    for st_path in &st_files {
...

        {
            ...
            if is_packed_mxfp4 {
                ...
                // FP8 block-quant pre-pass (Kimi-K2 / DeepSeek-V3 fp8): tensors
                // stored as (.weight: F8_E4M3, .weight_scale_inv: F32 [N/128, K/128]).
                // Consumes both names so the main loop skips them.
                let fp8_block_consumed =
                    dequantize_fp8_block_companions(&st, prefixes, &mut tensors)?;
                ...
                for (name, view) in st.tensors() {
                    ...
                    if fp8_block_consumed.contains(&name) {
                        continue;
                    }
